...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        double totalElectricBill;
        if (unitsUsed <= BASIC_SERVICE_LIMIT) {
            return unitsUsed * BASIC_SERVICE_RATE;
        } else {
            totalElectricBill = BASIC_SERVICE_LIMIT * BASIC_SERVICE_RATE;
            return totalElectricBill + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE);
        }
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        double energyDiscount;
        double total;
        double totalElectricBill;
        if (unitsUsed <= BASIC_SERVICE_LIMIT) {
            total = unitsUsed * BASIC_SERVICE_RATE;
        } else {
            totalElectricBill = BASIC_SERVICE_LIMIT * BASIC_SERVICE_RATE;
            total = totalElectricBill + ((unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE);
        }
        if (hasRenewableEnergy) {
            total = total - (RENEWABLE_ENERGY_DISCOUNT * total);
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        } else if (netUsage > 100.0) {
            totalElectricBill = 100.0 * 0.2;
            total = totalElectricBill + ( netUsage - 100.0) * 0.25;
        ...
        if (unitsReturned > 0) {
            ...
            energyDiscount = 0.05 * total;


            total = total - energyDiscount;
