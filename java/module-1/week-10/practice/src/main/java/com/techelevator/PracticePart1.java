...
public class PracticePart1<result, x, y> {
	...
	*/
	public String Q06_stringBetweenTags(String stringToSearch, String tag) {
		...
		if (!stringToSearch.contains(tag)) {
			return "";
		} else if (stringToSearch.lastIndexOf(tag) == stringToSearch.indexOf(tag)) {
			return "";
		}
		int tagStart = stringToSearch.indexOf(tag);
		int tagEnds = stringToSearch.lastIndexOf(tag);
		int tagLength = stringToSearch.length();


		return stringToSearch.substring(tagStart + tagLength,tagEnds);
	}
	//for (int i = 0; i < stringToSearch.length(); i++) {
//			if (tag != null) {
//				return stringToSearch.substring(3, stringToSearch.length() - 3);
//			}
//
//		}
	...
	*/
	...
	rather than "paid out" in lump sums.
	...
	*/
	public List<Integer> Q10_reverseList(List<Integer> numbers) {
		...
		for(int i = numbers.size()-1; i >= 0; i--){
			reverseNumberList.add(numbers.get(i));
		}
	...
	public Map<String, Integer> Q11_idealPets(String[] suggestedPets) {
        ...
        for(String pet: suggestedPets){
         if (map.containsKey(pet)){
             int counter = map.get(pet);
             map.put(pet, counter +1);
		 } else{
              map.put(pet, 1);
		 }
		}
